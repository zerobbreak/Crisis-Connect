...
// Match your backend alert structure
...
export default function StatisticsPage() {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        const res = await fetch("http://localhost:8000/alerts/history?limit=200");
        if (!res.ok) throw new Error("Failed to fetch alerts");
        const data = await res.json();
        const mapped: DisplayAlert[] = data.alerts.map((a: BackendAlert) => ({
          id: a._id || `alert-${Math.random()}`,
          ...
          severity: mapRiskLevel(a.risk_level),
          riskScore: Math.round(a.composite_risk_score || 0)
        }));
        setAlerts(mapped);
      } catch (err) {
    ...
    fetchAlerts();
    const interval = setInterval(fetchAlerts, 300000); // Refresh every 5 mins
  ...
  // Fetch weather only if we have a location
  useEffect(() => {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lastLocation.lat}&lon=${lastLocation.lng}&appid=${apiKey}&units=metric`)
      .then(r => r.ok ? r.json() : Promise.reject('Weather API error'))
      .then(setWeather)
  ...
  // Trend: Alerts per day (last 14 days)
  const trend = useMemo(() => {
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white border rounded-xl p-4">
          ...
          {Object.keys(totals.bySeverity).length === 0 ? (
            ...
            <SimpleBarChart data={Object.entries(totals.bySeverity).map(([label, value]) => ({ label, value }))} />
          )}
        </div>
...
function mapRiskLevel(level: string): string {
  ...
  return map[level.toUpperCase()] || 'medium';
}
